...
module.exports = {
  config: {
    ...
    category: "ai",
    guide: {
      en: "{pn} [prompt] - Generate image using Flux AI"
    }
  },
  ...
  onStart: async function({ event, message, args, api }) {
    try {
      ...
      if (args.length === 0) {
        return message.reply("⚠️ Please provide a prompt to generate an image. Example: /flux a beautiful landscape");
      }
      ...
      // API URL with your provided key
      const apiUrl = `https://kaiz-apis.gleeze.com/api/flux?prompt=${encodedPrompt}&apikey=a96577fa-b30a-428a-af48-b2e816c16a4c`;
      
      // Download the image
      const imagePath = path.join(__dirname, "tmp", "flux_image.jpg");
