	...
	if (threadID) {
		try {
			...
			if (!findInCreatingThreadData) {
				...
				// queue register to avoid duplicate create
				...
				try {
					...
					// Auto-approve & fill safe fields so bot can respond immediately
					...
					await threadsData.set(threadID, {
						...
						settings: {
							...(threadData?.settings || {}),
							approval: { status: true, approvedBy: "system", at: Date.now() }
						}
					}).catch(() => {});
